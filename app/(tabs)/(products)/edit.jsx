  ...
  const fetchProductDetails = async (id) => {
    try {
      ...
      const response = await axios.get(
        `https://virtserver.swaggerhub.com/RACHITGARG258/simple-api/1.0.0/products/${id}`
      );
      const product = response.data;

      setImage(product.productImage);
      setProductName(product.productName);
      setProductHashId(product.productId);
      setProductWeight(product.productWeight.toString());
      setProductPrice(product.productPrice.toString());
      setProductDescription(product.productDescription);
      setImageOne(product.productMedia[0]);
      setImageTwo(product.productMedia[1]);
      setImageThree(product.productMedia[2]);
      setProductBrand(product.organizationBrand);
      setProductCategory(product.organizationCategory);
      setProductCollection(product.organizationCollection);
        ...
        <View style={styles.topContainer}>
          <View style={[styles.flex, { marginBottom: 16 }]}>
            <AntDesign
              ...
              onPress={() => router.back()}
            />
            <Text style={styles.screenName}>Edit Product</Text>
          </View>
